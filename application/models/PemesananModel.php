...
class PemesananModel extends CI_Model
{
    ...
    public function count_total(array $criteria = null)
    {
     ...
     public function totalBayar($IdLaundry,$date_awal,$date_akhir)
    {
        // $this->db->select ('*');
        // $this->db->from('pemesanan');
        // $this->db->where('is_bayar',1);
        // $query= $this->db->get();

        $sql = "SELECT SUM(jml_bayar) as totalBayar FROM `pemesanan` WHERE `is_bayar` = 1 && `laundry_id` = $IdLaundry && `tanggal_masuk` BETWEEN '$date_awal' AND '$date_akhir'";
        // print_r($sql);
        // exit();
    ...
    public function tampil($IdLaundry,$date_awal,$date_akhir)
    {
        // $this->db->select ('*');
        // $this->db->from('pemesanan');
        // $this->db->where('is_bayar',1);
        ...
        $sql = "SELECT * FROM `pemesanan` WHERE `is_bayar` = 1 && `laundry_id` = $IdLaundry && `tanggal_masuk` BETWEEN '$date_awal' AND '$date_akhir'";
        // print_r($sql);
        // exit();
        $data = $this->db->query($sql);
        return $data;
