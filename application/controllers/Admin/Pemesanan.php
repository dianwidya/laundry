...
class Pemesanan extends MY_Controller
{
    ...
    public function get_paket()
    {
        ...
        $cek_member = $this->customer->find($where)->row();
        // $cek_member->is_member
        // echo json_encode($cek_member);exit;
        if($cek_member->is_member == 0){
        ...
        foreach($dataPaket as $p){
            if($cek_member->paket_id == $p->id) {
                $html .= "<option value='".$p->id."' selected>".$p->name.' - '.$ket."</option>";    
            } else {
